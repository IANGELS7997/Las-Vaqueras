...
export function ProductCard({ item, onAdd, outOfStock, closed }: ProductCardProps) {
  ...
  const locked = Boolean(outOfStock || closed);
  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden" onClick={() => !locked && onAdd(item)}>
        ...
        {closed && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/55">
            <span className="rounded-lg bg-black/70 px-3 py-1.5 text-xs font-bold text-white">
              CERRADO
            </span>
          </div>
        )}
      ...
      <div className="p-3.5" onClick={() => !locked && onAdd(item)}>
